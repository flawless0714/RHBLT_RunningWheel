...
void wifiPosDataEncode(void) /* we are going to deprecate POS_4of4 since its value is too close, maybe is sample rate or resolution, but on volt meter it got almost same value */
{
	uint8_t index = 0;
	wifiSendBuffer[index++] = (runningWheel.wheel[0].currentPos ); // WARN: weAreNoLonger+1now
	wifiSendBuffer[index++] = (runningWheel.wheel[1].currentPos ); 
	wifiSendBuffer[index++] = (runningWheel.wheel[2].currentPos ); 
}
...
void wifiModuleInit(void)
{
	switch (wifi.Cstate)
	{
		...
		case CWSAP:
			...
			wifiCommandEncode(AT_CWSAP, "\"cLab-RunningWheel\",\"00001111\",9,3,2,0");
			uartSend(&wifiSendBuffer, TYPICAL_RECEIVE_SIZE);
			break;
		case CIPMUX:
			if ((wifi.currentTick + TYPICAL_WAIT_TIME) >= mcu.sysTick)	return;
